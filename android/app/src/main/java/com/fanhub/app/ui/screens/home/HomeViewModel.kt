...
data class HomeUiState(
    val featured: List<Video> = emptyList(),           // 精选（播放量最高）
    val myFavorites: List<Video> = emptyList(),        // 我的收藏
...
class HomeViewModel @Inject constructor(
    ...
    fun loadData() {
        viewModelScope.launch {
            ...
            try {
                // 1. 精选：播放量最高的视频
                val featuredResp = apiService.getVideos(
                    page = 1, 
                    perPage = 8, 
                    sortBy = "view_count", 
                    order = "desc"
                )
                _uiState.update { it.copy(featured = featuredResp.items) }
                ...
                val favorites = favoritesResp.items
