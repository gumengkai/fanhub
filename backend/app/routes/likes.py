...
def get_likes():
    """Get all liked items with pagination."""
    ...
    if media_type is None or media_type == 'video':
        videos = Video.query.filter_by(is_liked=True).order_by(desc(Video.updated_at)).all()
...
def get_likes_stats():
    ...
    video_count = Video.query.filter_by(is_liked=True).count()
